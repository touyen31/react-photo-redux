            ...
            <div style={{ width: '100%', height: 590, display: 'flex' }}>
                ...
                <div style={{ flex: 2, backgroundColor: '#cecece', borderLeftStyle: 'solid', borderLeftWidth: 1, paddingLeft: 10, paddingRight: 10, overflow: 'scroll' }}>
                    ...
                    <div style={{ width: '100%', display: 'grid', gridTemplateColumns: '170px 170px 170px', paddingBottom: 10, justifyContent: 'center' }}>
                        {tag.map((item, index) =>
                            <div key={index} style={{ backgroundColor: '#a4cdfc', borderRadius: 10, marginLeft: 5, marginRight: 5, marginTop: 5, textAlign: 'center', overflow: 'hidden', fontSize: 16 }} onClick={() => {this.props.history.push(`/search`); this.props.updatetext(item._content); this.props.search(item._content)}}>{item._content}</div>
                        )}
                    </div>
                </div>
            </div>
